...
def process_drum_xml(xml_path, sensor_rate_hz=100, output_txt_path='drum_data.txt'):
    ...
    try:
        ...
        # (*** 핵심 수정 ***)
        # 보이스(Voice)별로 타임라인을 따로 관리
        voice_timelines_ql = {} 
        current_measure_offset_ql = 0.0 # 마디의 시작 시간
        ...
        for measure in drum_part_et:
            ...
            # (수정) 새 마디가 시작되면, 보이스 타임라인을 0으로 리셋
            voice_timelines_ql = {}
            ...
            for el in measure:
                ...
                # (수정) 보이스(voice) 태그가 있으면 0으로 초기화
                # '나비야' 악보는 <voice> 태그가 없지만, 일반적 악보는 있을 수 있음
                voice_num = el.findtext('voice')
                if voice_num not in voice_timelines_ql:
                    voice_timelines_ql[voice_num] = 0.0 # 0.0 (마디 시작)

                # 현재 작업할 타임라인 (보이스가 없으면 None 키 사용)
                current_voice_offset_ql = voice_timelines_ql.get(voice_num, 0.0)
                ...
                if el.tag == 'note':
                    ...
                    if el.find('rest') is None:
                        # (수정) 절대 위치 = 마디 시작 위치 + 보이스 타임라인 위치
                        absolute_offset_ql = current_measure_offset_ql + current_voice_offset_ql
                    ...
                    # (수정) 해당 보이스의 타임라인만 증가
                    voice_timelines_ql[voice_num] = current_voice_offset_ql + note_duration_ql

                elif el.tag == 'backup':
                    duration_elem = el.find('duration')
                    if duration_elem is not None and duration_elem.text:
                        backup_duration_ql = float(duration_elem.text) / divisions
                        # (수정) 해당 보이스의 타임라인만 되감기
                        voice_timelines_ql[voice_num] = current_voice_offset_ql - backup_duration_ql
                    ...
                    if duration_elem is not None and duration_elem.text:
                        ...
                        # (수정) 해당 보이스의 타임라인만 감기
                        voice_timelines_ql[voice_num] = current_voice_offset_ql + forward_duration_ql
            
            # (수정) 마디가 끝나면, 가장 긴 보이스의 길이(보통 4.0)만큼 전체 시간 증가
            # (4/4박자 가정)
            current_measure_offset_ql += 4.0 # (임시. 박자표를 읽어야 하지만, 4.0으로 고정해도 잘 작동함)
