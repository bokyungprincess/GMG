...
def process_drum_xml(xml_path, sensor_rate_hz=100, output_txt_path='drum_data.txt'):
    ...
    # 6. (*** 수정됨 ***)
    # 비트 배열 3종 채우기 (MIDI 맵 기준)
    print("마디(Measure)를 기준으로 노트 오프셋 및 MIDI 번호를 분석합니다...")
    ...
    for measure in all_measures:
        ...
        for el in elements_in_measure:
            ...
            if (not el.isRest) and hasattr(el, 'duration') and hasattr(el, 'offset'):
                ...
                # (신규) 이 노트(el)가 어떤 악기인지 MIDI 번호로 확인
                
                # 체크할 MIDI 노트 번호 리스트
                midi_notes_to_check = []

                if el.isChord:
                    # 동시 타격(코드)인 경우: 모든 음의 MIDI 번호 추출
                    for p in el.pitches:
                        if hasattr(p, 'midi'):
                            midi_notes_to_check.append(p.midi)
                elif hasattr(el, 'pitch') and hasattr(el.pitch, 'midi'):
                    # 단일 타격(노트)인 경우: 해당 음의 MIDI 번호 추출
                    midi_notes_to_check.append(el.pitch.midi)

                # MIDI 번호 리스트를 순회하며 맵과 비교
                for midi_val in midi_notes_to_check:
                    if midi_val in MIDI_MAP['kick']:
                        beat_array_kick[start_index] = 1
                        kick_count += 1
                    elif midi_val in MIDI_MAP['snare']:
                        beat_array_snare[start_index] = 1
                        snare_count += 1
                    elif midi_val in MIDI_MAP['hit']:
