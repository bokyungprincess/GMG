...
# (신규) 악보상 '표시 위치' 맵
# -------------------------------------------------------------------
# (키: 악기 유형, 값: (displayStep, displayOctave) 튜플 리스트)
# '질풍가도.xml' 및 '나비야.xml' 원본 파일 기준입니다.
DISPLAY_MAP = {
    'kick': [
    ...
    # (탐(Tom) 종류는 의도적으로 제외했습니다. - 예: ('E', 5), ('D', 5), ('A', 4))
}
# -------------------------------------------------------------------
...
def process_drum_xml(xml_path, sensor_rate_hz=100, output_txt_path='drum_data.txt'):
    ...
    (*** 핵심 로직 수정: Display Step/Octave 맵 사용 ***)
    """
    try:
        # 1. MusicXML 파일 파싱
    ...
    # 6. (*** 수정됨 ***)
    # 비트 배열 3종 채우기 (Display Step 맵 기준)
    ...
    for measure in all_measures:
        ...
        for el in elements_in_measure:
            ...
            # (신규) 이 요소(el)가 어떤 악기인지 위치로 확인
            
            # 1. 동시 타격(Chord)인 경우 (예: '질풍가도' 15마디)
            if el.isChord:
                for n in el.notes: # Chord 안의 개별 노트(n)를 순회
                    if hasattr(n, 'unpitched') and n.unpitched is not None:
                        step = n.unpitched.displayStep
                        octave = n.unpitched.displayOctave
                        pos = (step, octave)
            ...
            # 2. 단일 타격(Note/Unpitched)인 경우
            elif (not el.isRest) and hasattr(el, 'unpitched') and el.unpitched is not None:
                step = el.unpitched.displayStep
                octave = el.unpitched.displayOctave
                pos = (step, octave)
