...
def list_xml_files(search_path):
    """
    (수정됨) 지정된 디렉토리의 XML 파일들을 나열하는 함수
    
    Args:
        search_path (str): 검색할 폴더 경로
    """
    extensions = ('.xml', '.mxl', '.musicxml')
    try:
        # os.listdir('.') 대신 search_path를 사용
...
def process_drum_xml(xml_path, sensor_rate_hz=100, output_txt_path='drum_data.txt'):
    ...
    # 6. 비트 배열 채우기 (*** 핵심 수정: Measure를 기준으로 offset 재계산 ***)
    note_found_count = 0
    print("마디(Measure)를 기준으로 노트 오프셋을 재계산합니다...")

    # 드럼 파트에서 모든 '마디(Measure)'를 가져옴
    try:
    ...
    for measure in all_measures:
        ...
        for el in elements_in_measure:
            ...
            if (not el.isRest) and hasattr(el, 'duration') and hasattr(el, 'offset'):
                # 이 노트의 시작 위치 (마디 기준)
                note_local_offset = float(el.offset)
                
                # (중요) 노트의 '절대' 시작 위치
                absolute_offset_ql = measure_start_offset + note_local_offset
                ...
                if 0 <= start_index < array_length:
                    ...
                    # (디버깅)
                    # print(f"  [디버그] 노트 (Abs Offset: {absolute_offset_ql})가 배열 범위 밖입니다. (index: {start_index} vs array_len: {array_length})")
                    pass

    # 6-1. 마킹된 '1'의 총 개수를 다시 세서 정확한 카운트 확보
    final_marked_count = sum(beat_array)

    if final_marked_count == 0:
        print("경고: 노트 위치를 배열에 마킹하지 못했습니다. (배열이 0일 수 있음)")
    else:
        print(f"총 {final_marked_count}개의 타임슬롯을 배열에 마킹했습니다.")
        ...
        while True: 
            try:
                ...
                if 0 <= choice < len(xml_files_list):
                    # (*** 수정됨 ***)
                    # 선택한 파일 이름과 검색 경로를 조합하여 '절대 경로'를 만듭니다.
