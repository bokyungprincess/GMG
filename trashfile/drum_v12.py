...
def process_drum_xml(xml_path, sensor_rate_hz=100, output_txt_path='drum_data.txt'):
    ...
    (*** 핵심 로직 수정: Display Step/Octave(string) 맵 사용 ***)
    """
    try:
        # 1. MusicXML 파일 파싱
    ...
    # 6. (*** 수정됨: Chord 내부 노트 탐색 로직 변경 ***)
    print("마디(Measure)를 기준으로 노트 오프셋 및 Display Step을 분석합니다...")
    ...
    for measure in all_measures:
        ...
        for el in elements_in_measure:
            ...
            # (신규) el이 Chord일 때와 Note일 때를 분리하여 'unpitched' 정보 탐색
            
            notes_to_check = []
            if el.isChord:
                # Chord인 경우, 하위 노트(Unpitched)들을 리스트에 담음
                # '질풍가도' 15마디(킥+스네어 동시)의 경우 el.notes[0] = 킥, el.notes[1] = 스네어
                for n in el.notes:
                    if hasattr(n, 'unpitched') and n.unpitched is not None:
                        notes_to_check.append(n.unpitched)
            elif (not el.isRest) and hasattr(el, 'unpitched') and el.unpitched is not None:
                # 단일 노트(Unpitched)인 경우
                notes_to_check.append(el.unpitched)

            # 수집된 'unpitched' 객체들을 검사
            for unpitched_note in notes_to_check:
                try:
                    step = unpitched_note.displayStep
                    octave = str(unpitched_note.displayOctave) # <-- 문자열(str)로 비교
                    pos = (step, octave)
                    ...
                    # print(f"[Debug] Unpitched note parse error: {e}")
                    pass # 오류가 나도 계속 진행
