...
# -------------------------------------------------------------------
# (수정됨) 악보상 '표시 위치' 맵 (ElementTree는 Octave를 문자로 읽음)
# -------------------------------------------------------------------
DISPLAY_MAP = {
    'kick': [
...
def find_drum_part_et(root):
    ...
    try:
        # P1 파트가 <clef><sign>percussion</sign></clef>을 가졌는지 확인
    ...
    return root.find('part') # P1이 아니더라도 첫 번째 파트 반환


def process_drum_xml(xml_path, sensor_rate_hz=100, output_txt_path='drum_data.txt'):
    """
    (*** 로직 전면 수정: ElementTree(ET)를 사용하여 XML 직접 파싱 ***)
    """
    ...
    try:
        ...
        divisions = 1.0 # 4분음표당 틱 수 (기본값)
        current_offset_ql = 0.0 # 현재 위치 (QuarterLength 기준)
        
        kick_count, snare_count, hit_count = 0, 0, 0

        # P1 파트의 모든 자식 요소(주로 <measure>)를 순회
        for measure in drum_part_et:
            if measure.tag != 'measure':
                continue # <measure>가 아니면 건너뜀

            # <measure> 태그가 시작되는 위치 = 이 마디의 시작 오프셋
            measure_start_offset_ql = current_offset_ql
            
            # 마디 내부의 모든 요소(<attributes>, <note>, <backup> 등)를 순회
            for el in measure:
                
                # 1. <attributes> -> <divisions> (틱 계산 기준)
            ...
            # (디버깅) 마디가 끝난 후 현재 오프셋이 마디 시작+마디 길이와 일치하는지 확인
            # (예: 4/4박자면 measure_start_offset_ql + 4.0 이 되어야 함)
            # print(f"Measure {measure.get('number')} 끝. 현재 오프셋: {current_offset_ql}")
