...
def save_formatted_xml(tree, output_path):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        # MusicXML DTD 선언 (원본 파일의 형식에 맞춰줌)
...
def merge_measures_by_type(input_dir):
    ...
    # 병합된 파일을 저장할 상위 디렉토리
    output_dir = os.path.abspath(os.path.join(input_dir, ".."))

    print(f"'{input_dir}' 디렉토리의 파일들을 병합하여 상위 폴더에 저장합니다...")

    # 4. 유형별로 병합 실행
    for file_type in types_to_merge:
        print(f"\n--- {file_type.upper()} 파일 병합 중 ---")
        
        # 4-1. 현재 유형에 맞는 파일만 필터링
